...
namespace Septa.PayamGostarClient.Initializer.Models.Customization.CrmObjectType
{
    public class PayamGostarCrmObjectTypeStageApiClient : BaseApiClient, IPayamGostarCrmObjectTypeStageApiClient
    {
        ...
        public CrmObjectTypeStageCreationResultDto Create(CrmObjectTypeStageCreationRequestDto request)
        {
            return SeptaKit.Extensions.SeptaKitTaskExtensions.RunSync(() => CreateAsync(request));
        }

        public IEnumerable<StageGetResultDto> GetStages(Guid crmObjectId)
        {
            return SeptaKit.Extensions.SeptaKitTaskExtensions.RunSync(() => GetStagesAsync(crmObjectId));
        }
