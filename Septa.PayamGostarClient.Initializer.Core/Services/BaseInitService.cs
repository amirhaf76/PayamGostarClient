...
namespace Septa.PayamGostarClient.Initializer.Core.Services
{
    public abstract class BaseInitService<T> : IInitService where T : BaseCRMModel
    {
        ...
        private readonly ICrmModelMatchingValidator _superCrmModelMatchingValidator;

        protected IPayamGostarCrmObjectTypeApiClient CrmObjectTypeApi { get; }


        internal BaseInitService(T intendedCrmObject, IInitServiceAbstractFactory factory)
        {
            IntendedCrmObject = intendedCrmObject;

            CrmObjectTypeApi = factory.PayamGostarApiClient.CustomizationApi.CrmObjectTypeApi;
            ...
            _superCrmModelMatchingValidator = factory.CreateSuperCrmModelMatchingValidator();
        ...
        {
            CheckSuperCrmObjectMatching(superCrmObject);
        ...
        {
            var receivedCrmObjects = await CrmObjectTypeApi.SearchAsync(request);

            var receivedCrmObject = receivedCrmObjects.FirstOrDefault();
        ...
        private void CheckSuperCrmObjectMatching(CrmObjectTypeSearchResultDto existedCrmObj)
        {
            try
            {
                _superCrmModelMatchingValidator.CheckMatchingBaseCrmObject(IntendedCrmObject, existedCrmObj);
            }
            catch (MisMatchException e)
            {
                throw new MisMatchException($"Mismatch in crm model with '{IntendedCrmObject.Code}' code!", e);
